  ...
  return (
    <section className="relative h-screen w-full overflow-hidden border-t border-border">
      ...
      <button
        ...
        className="absolute left-6 top-1/2 z-20 -translate-y-1/2 text-foreground/50 hover:text-foreground/90 transition-all duration-300 animate-[bounceLeft_2s_ease-in-out_infinite] backdrop-blur-sm bg-background/10 rounded-full p-2"
        onClick={() => {
      ...
      <button
        ...
        className="absolute right-6 top-1/2 z-20 -translate-y-1/2 text-foreground/50 hover:text-foreground/90 transition-all duration-300 animate-[bounceRight_2s_ease-in-out_infinite] backdrop-blur-sm bg-background/10 rounded-full p-2"
        onClick={() => {
          trackRef.current?.scrollBy({ left: isMobile ? window.innerWidth : window.innerWidth / 2, behavior: "smooth" });
        }}
      >
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 6 15 12 9 18" /></svg>
      </button>
